...
class ARFFReader( ) :
  ...
  def __init__( i, filename, ignoreCols=None, sep="," ) :
    ...
    for x in i.fileHandle : 
      ...
      elif( x.lower().startswith("@class" ) ):
        i.attr.append( "=" + x.strip().split()[1] )
  
    i.attr = [ x for idx, x in enumerate( i.attr ) if idx not in i.ignoreCols ]
  ...
  def __iter__ ( i ) :
    for line in i.fileHandle : 
      ...
      data = [ x for idx,x in enumerate(line.split(i.sep)) if idx not in i.ignoreCols ]
      yield Row( map( i.wrangle, data ) )
...
class Table(Vector):
  ...
  def getDependent( i ) : 
    index = [ x for x in range(len(i.header)) if i.header[x].isDep() ] 
  ...
  def getDependentValues( i , x ) :
    return [ x[y] for y in range(len(i.header)) if i.header[y].isDep() ] 
...
class Header(Vector) : 
  def __init__( i, col ) : 
    i.data = [ _Header(col[x], x) for x in range(len(col)) ]
...
class Num:
  ...
  def __init__( i ) :
    ...
    i.n       = 0
    i.m2      = 0
    i.max     = -100000000000000000
    i.min     = 100000000000000000
